    ...
    public class PrefabImagePairManager : MonoBehaviour, ISerializationCallbackReceiver
    {
        ...
        private void UpdatePrefab(ARTrackedImage trackedImage)
        {
            if (m_Instantiated.TryGetValue(trackedImage.referenceImage.guid, out var prefab))
            {
                ...
                {
                    // Enable the prefab and update its position/rotation
        ...
        private void RemovePrefab(KeyValuePair<TrackableId, ARTrackedImage> trackedImagePair)
        {
            var trackedImage = trackedImagePair.Value; // Extract the ARTrackedImage from the KeyValuePair

            if (m_Instantiated.TryGetValue(trackedImage.referenceImage.guid, out var prefab))
            {
                Destroy(prefab); // Destroy the instantiated prefab
                m_Instantiated.Remove(trackedImage.referenceImage.guid); // Remove it from the dictionary
            }
        }
